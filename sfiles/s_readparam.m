function input = s_readparam(format, varargin)
% parse input arguments
%
% ### Syntax
%
% `parsed = s_readparam(format,Name,Value)`
%
% ### Description
%
% `parsed = s_readparam(format,Name,Vale)` parses name-value pair
...
%
% `s_readparam` is used in most of the method functions of [spinw].
%
% ### Input Arguments
%
% `format`
...
if nargin == 2 && ischar(varargin{1}) && varargin{1} == '-'
    ...
    warning('s_readparam:ClipboardCopy','Input parameter names are copied to the clipboard!');
end
...
elseif nargin == 1
    ...
    MException('s_readparam:WrongParameter','Parameter name-value pairs are expected!').throwAsCaller;
end
    ...
    if any(rawIdx)
        ...
        if inputValid
            ...
            if isfield(format,'soft') && format.soft{ii}
                ...
                MException('s_readparam:ParameterSizeMismatch',['Input parameter size mismatch in parameter ''' fName{ii} '''!']).throwAsCaller;
            end
        end
    else
        if isfield(format,'defval') && (any(size(format.defval{ii})) || (isfield(format,'soft') && format.soft{ii}))
            ...
            MException('s_readparam:MissingParameter',['Necessary input parameter ''' fName{ii} ''' is missing!']).throwAsCaller;
        end
    end
end
...
if ~all(usedField)
    ...
    warning('s_readparam:UnreadInput','Unregognised input parameter%s!',wName);
end
