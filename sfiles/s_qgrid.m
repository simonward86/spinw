function qGrid = s_qgrid(varargin)
% creates a Q grid
% 
% ### Syntax
% 
% `qgrid = s_qgrid(Name,Value)`
% 
% ### Description
% 
% `qgrid = s_qgrid(Name,Value)` generates n-dimensional grids ($n<=3$) in
...
if isempty(param.bin)
    ...
    if ~ismember(double(ebin),[1 0 0;1 1 0;1 1 1],'rows')
        error('s_qgrid:WrongInput','Wrong input!')
    end
...
for ii = 1:nDim
    ...
    switch numel(bin0)
        ...
        otherwise
            if isempty(param.bin)
                error('s_qgrid:WrongInput','Each bin has to be 1, 2 or 3 element vector!')
            end
            v{ii} = bin0;
    end
end
