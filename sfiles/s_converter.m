function out = s_converter(value, unitIn, unitOut, particleName,invert)
% converts energy and momentum units for a given particle
% 
% ### Syntax
% 
% `out = s_converter(value, unitIn, unitOut)`
% 
% `out = s_converter(value, unitIn, unitOut, particleName)`
%
% ### Description
% 
% `out = s_converter(value, unitin, unitout)` will convert momentum and
% energy values assuming neutron as a particle.
% 
% `out = s_converter(value, unitin, unitout,particleName)` will convert
...
elseif isempty(unitIn) || isempty(unitOut)
    error('s_converter:WrongInput','One of the unit is empty string!')
end
...
switch particleName
    ...
    case {'photon' 'xray' 'x-ray' 'light'}
        m = 0;
    otherwise
        error('s_converter:WrongParticle','Particle name is wrong!');
end
...
switch unitIn
    ...
    case 'mps'
        if m~=0
            ...
            error('s_converter:WrongUnit','Speed cannot be an input for photon!');
        end
    case 'meV'
        if m~=0
