...
if ~isdeployed
    ...
    if nargout == 0
        if nField == 0
            ...
            if any(revNum)
                fprintf('This version of Spin (rev. num. %d) is not released yet!\n',revNum);
            else
                fprintf('This version of Spin is not released yet!\n');
            end
        else
            disp([verStruct.Name verStruct.Version ' (rev ' num2str(verStruct.Revision) ')']);
            onlineRev = sw_update;
            if onlineRev > str2num(verStruct.Revision) %#ok<ST2NM>
                disp(['Newer version of Spin is available online (rev. num. ' num2str(onlineRev) '), use the sw_update() function to download it!']);
            else
