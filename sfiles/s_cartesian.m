function [vyOut, vzOut, vxOut] = s_cartesian(n)
% creates a right handed Cartesian coordinate system
% 
% ### Syntax
% 
% `[vy, vz, vx] = s_cartesian(n)`
%
% `V = s_cartesian(n)`
% 
% ### Description
% 
% `[vy, vz, vx] = s_cartesian(n)` creates an $(x,y,z)$ right handed
% Cartesian coordinate system with $v_x$, $v_y$ and $v_z$ defining the
% basis vectors. 
%
% `V = s_cartesian(n)` the generated basis vectors are stored in the `V`
...
elseif all(size(n) == [3 3])
    if det(n) == 0
        error('s_cartesian:WrongInput','The input vectors are not linearly independent!')
    end
    ...
    error('s_cartesian:WrongInput','Wrong size of n!')
end
