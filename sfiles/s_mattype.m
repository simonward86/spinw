function type = s_mattype(mat, epsilon)
...
% `type = s_mattype(mat)`
% 
% `type = s_mattype(mat,epsilon)`
%
% ### Description
% 
% `type = s_mattype(mat)` determines the type of the input matrix `mat`
...
if any(mSize(1:2)-[3 3])
    error('s_mattype:InputError','Dimensions of the Input matrix is not 3x3xN!');
end

if ~isreal(mat) && ~isa(mat,'sym')
    error('s_mattype:InputError','Input matrix is not real or symbolic!');
end
