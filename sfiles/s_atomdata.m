function varargout = s_atomdata(atomSymb, datType)
% returns information on chemical elements
% 
% ### Syntax
% 
% `data = s_atomdata(atomsymb)`
% 
% `data = s_atomdata(atomsymb,datatype)`
%
% ### Description
% 
% `data = s_atomdata(atomsymb)` returns information on chemical elements
...
% 
% `data = s_atomdata(atomsymb,datatype)` returns only the requested type
% of data.
%
% ### Examples
%
...
atom  = sw_readtable([s_rootdir 'dat_files' filesep 'atom.dat']);
...
switch datType
    ...
    otherwise
        error('s_atomdata:WrongInput','datType has to be one of the string options, see help s_atomdata!');
end
