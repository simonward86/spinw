...
% ### Description
% 
% `bc = s_nb(atomname)` returns the bound coherent neutron scattering
...
if iscell(atomName)
    ...
    ffPath = [s_rootdir 'dat_files' filesep 'isotope.dat'];
    ...
    if any(idx == numel(sigma))
        ...
        warning('s_nb:WrongInput','The neutron scattering length for %s is undefined, constant 1 will be used instead!',atomName{fIdx(1)})
    end
else
