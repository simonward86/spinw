...
% ### Description
% 
% `l = s_bonddim(c, {natom})` splits the given periodic bond network into
...
% input check
if size(C,1)~=5
    error('s_bonddim:WrongInput','The given bond matrix has wrong dimensions!')
end
% all values have to be integer
if any(floor(C(:))-ceil(C(:)))
    error('s_bonddim:WrongInput','The given bond matrix must contain only integers!')
end
