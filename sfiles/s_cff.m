function [formFactVal, coeff] = s_cff(atomName, Q)
% returns the atomic charge form factor values for X-ray scattering
% 
% ### Syntax
% 
% `[formfactval, coeff] = s_cff(atomname, {q})`
% 
% ### Description
% 
% `[formfactval, coeff] = s_cff(atomname, {q})` returns the atomic charge
...
if iscell(atomName)
    ...
    ffPath = [s_rootdir 'dat_files' filesep 'xrayion.dat'];
    ...
    if any(idx == numel(formFact))
        ...
        warning('s_cff:WrongInput',['The x-ray scattering form factor for %s is undefined,'...
...
elseif size(atomName,2) == 11
    ...
    error('s_cff:WrongInput','Wrong input!')
end
