function [formFactVal, coeff, S] = s_mff(atomName, Q, nCoeff)
% returns the magnetic form factor values and coefficients
% 
% ### Syntax
% 
% `[~, coeff, s] = s_mff(atomname)`
% 
% `[formfactval, coeff, s] = s_mff(atomname,Q)`
%
% ### Description
% 
% `[~, coeff, s] = s_mff(atomname)` returns the magnetic form
...
% factor coefficients.
%
% `[formfactval, coeff, s] = s_mff(atomname,Q)` also calculates the form
...
if iscell(atomName)
    ...
    ffPath = [s_rootdir 'dat_files' filesep 'magion.dat'];
    ...
    if any(idx == numel(formFact))
        ...
        warning('s_mff:WrongInput','The magnetic form factor for %s is undefined, constant 1 will be used instead!',atomName{fIdx(1)})
    end
elseif size(atomName,2) >= 9
    % just calculates the form factor values
    coeff = atomName;
else
    error('s_mff:WrongInput','Wrong input!')
end
...
if ~isempty(Q)
    if size(Q,1)==3
        Q = sqrt(sum(Q.^2,1));
    elseif size(Q,1)~=1
        error('s_mff:WrongInput','The dimensions of the given Q array is wrong!')
    end
    Qs = Q(:)'/(4*pi);
