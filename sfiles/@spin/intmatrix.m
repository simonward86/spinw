function [SS, SI] = intmatrix(obj, varargin)
% generates interaction matrix
% 
% ### Syntax
% 
% `[SS, SI] = intmatrix(obj,Name,Value)`
% 
% ### Description
% 
% `[SS, SI] = intmatrix(obj,Name,Value)` lists the bonds and generates
...
%   points from `atom1` to `atom2`, for details see [spin.coupling].
...
%               [spin.matom] list. The following rows contains the
...
%               `spin.coupling.idx`). The `dip` field contains the dipolar
...
%        warning('spin:intmatrix:symmetry',['The non-isotropic symbolic matrices '...
%            'will be rotated unsing the point group operators, the result can be ugly!']);
%    end
%end


inpForm.fname  = {'fitmode' 'plotmode' 'zeroC' 'conjugate' 'sortDM'};
inpForm.defval = {false     false      false   false       false   };
...
if obj.sym
    ...
    isHeis = s_mattype(obj.matrix.mat,0) == 1;
...
if ~param.fitmode
    ...
    if any(JJ.type(mat_type==1)~=1)
        error('spin:intmatrix:DataError','Biquadratic exchange matrix has to be isotropic!')
    end
