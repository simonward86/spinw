...
% whether a space group is defined call the [spin.symmetry] function.
%
% {{warning This function has to be used after the crystal structure is defined.
%   The [spin.addcoupling] function will only work afterwards. }}
%
% ### Examples
%
% A triangular lattice is generated using `spin.gencoupling` and
% the [spin.table] function lists the 1st, 2nd and 3rd neighbor bonds:
%
% ```
% >>cryst = spin
...
%   if they are given ([spin.symmetry] returns `true`).
%
% `'maxDistance'`
% : Maximum bond length that will be stored in the
%   [spin.coupling] property in units of \\ang. Default value is 8.
%
% `'maxSym'`
...
if nMagAtom == 0
    error('spin:gencoupling:NoMagAtom','There is no magnetic atom (S>0) in the unit cell!');
end
...
if cMat(6,1) < param.dMin
    error('spin:gencoupling:AtomPos',['Some atoms are too close (Dmin=' ...
...
if isSym
    ...
    while ii <= maxidxSym
        ...
        while size(sortMs,2)>0
            ...
            genC(:,~unC) = [];
            if sum(~iNew) ~= sum(unC)
                error('spin:gencoupling:SymProblem','Symmetry error! ii=%d, idx=%d. Try to change ''tol'' parameter.',ii,idx);
            end
...
feval(sprintf('%s.validate',class(obj)),obj)
