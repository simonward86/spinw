...
%
% Alternatively the lattice parameters can be given directly when the spin
% object is created using `spin(inpStr)` command, where struct contains
...
% 
% `lat_const`
% : `[a, b, c]` lattice parameters in units defined in [spin.unit] (with \\ang
...
%   [spin.gencoupling] function will determine the equivalent bonds based on
%   bond length.
% 
% `label`
...
% 
% `R`
% : Rotation matrix that brings the input basis vector to the Spin
%   compatible form:
%   ```
%   BVspin = R*BV
%   ```
% 
% The result of the `spin.genlattice` function is that `obj.lattice` field
...
% ### See also
%
% [spin], [swsym.add], [swsym.operator], [spin.gencoupling]
%
...
if ~isempty(param.bv)
    ...
    % the new abc vectors compatible with the Spin format
    a = BV2(:,1);
    b = BV2(:,2);
    c = BV2(:,3);
