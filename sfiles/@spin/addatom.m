...
%   for details see [s_mff]. Also string labels can be used from the
%   `magion.dat` file.
% 
% `formfactn`
...
%   see [s_cff], also labels can be used from the `xrayion.dat` file.
% 
% `Z`
...
% ### Output Arguments
% 
% The function modifies the [spin.unit_cell] property of the obj
% [spin] object.
% 
% ### See Also
% 
% [spin.genlattice] \| [spin.addmatrix] \| [swplot.color] \| [s_mff] \| [s_cff]
%
...
newAtom = s_readparam(inpForm, varargin{:});
...
if isempty(newAtom.label)
    ...
    warning('off','s_mff:WrongInput');
    warning('off','s_cff:WrongInput');
    warning('off','s_nb:WrongInput');
...
elseif numel(newAtom.biso)~= nNewAtom
    error('spin:addatom:WrongInput','Wrong input for option ''biso''!');
end

% check oxidation number
if ~isempty(newAtom.ox)
    if numel(newAtom.ox) ~= nNewAtom
        error('spin:addatom:WrongInput','Wrong input for option ''ox''!')
    end
end
...
if ~isempty(newAtom.Z)
    if numel(newAtom.Z) ~= nNewAtom
        error('spin:addatom:WrongInput','Wrong input for option ''Z''!')
    end
end
...
if ~isempty(newAtom.A)
    if numel(newAtom.A) ~= nNewAtom
        error('spin:addatom:WrongInput','Wrong input for option ''A''!')
    end
end
...
if isempty(newAtom.label)
    ...
    if ~isempty(newAtom.Z)
        newAtom.label = s_atomdata(newAtom.Z,'name');
    else
...
elseif isempty(newAtom.Z) || isempty(newAtom.ox)
    ...
    if isempty(newAtom.Z)
        newAtom.Z = s_atomdata(newAtom.name,'Z');
    end
end

% Generate atom colors
if isempty(newAtom.color)
    newAtom.color = s_atomdata(newAtom.Z,'color');
else
...
if isempty(newAtom.formfactn)
    [~,newAtom.ffn,newAtom.S0] = s_mff(newAtom.label,[],11);
...
if iscell(newAtom.formfactn)
    [~,newAtom.ffn] = s_mff(newAtom.formfactn,[],11);
...
if isempty(newAtom.formfactx)
    ...
    [~,newAtom.ffx]            = s_cff(newAtom.label);
end
...
if iscell(newAtom.formfactx)
    [~,newAtom.ffx] = s_cff(newAtom.fromfactx);
...
newAtom.b(1,:) = s_nb(newAtom.label);
...
feval(sprintf('%s.validate',class(obj)),newObj,'unit_cell')
...
if any(accumarray(rIdx,obj.unit_cell.occ)>1)
    warning('spin:addatom:WrongInput','Occupancy on some site is larger than 1!')
end
