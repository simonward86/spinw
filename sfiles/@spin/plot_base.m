function varargout = plot_base(obj,fn_wrapper,varargin)
% plots 3D model
% 
% ### Syntax
% 
...
if numel(varargin) == 1
    ...
    if ~isstruct(varargin{1})
        error('spin:plot:WrongOptions','Expected option-value pairs')
    end
...
param = s_readparam(inpForm, optNo{:});
...
warn0 = warning('query');
warning('off','s_readparam:UnreadInput')
warning('off','plotbond:EmptyPlot')

paramG = s_readparam(inpForm, varargG{:});

% sort options according to plot name
plotName = fn_wrapper.plotName;
nFun     = numel(plotName);
...
warning('on','s_readparam:UnreadInput')
