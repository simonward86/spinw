classdef spin < handle & matlab.mixin.SetGet
    ...
    % `obj = spin(source)`
    %
    % `obj = spin(figure_handle)`
    %
    % ### Description
    %
    % `obj = spin` constructs an empty spin class object.
    %
    % `obj = spin(obj)` constructs a spin class object from the
    % parameters defined in `obj`. If `obj` is spin class, it only checks
    % the integrity of its internal data structure. If `obj` is struct
    % type, it creates new spin object and checks data integrity.
    %
    % `obj = spin(source)` construct new spin class object, where
    % `source` is either a file path pointing to a local `cif` or `fst`
    % file or a link to an online file.
    %
    % `obj = spin(figure_handle)` copy the spin object stored in a
    % previous structural 3D plot figure, referenced by `figure_handle`.
    %
    %
    % The data structure within the spin object can be accessed by using
    % [spin.struct] method. All fields of the struct type data behind the
    % spin object are accessible through the main field names of the `obj`
    ...
    % ```
    %
    % spin is a handle class, which means that only the handle of the
    % object is copied in an assinment command `swobj1 = swobj2`. To create
    % a copy (clone) of an spin object use:
    %
    % ```
    % swobj1 = swobj2.copy
    % ```
    ...
    % ### Properties
    %
    % The data within the `spin` object is organized into a tree structure
    % with the main groups and the type of data they store are the
    % following:
    %
    % * [spin.lattice] unit cell parameters
    % * [spin.unit_cell] atoms in the crystallographic unit cell
    % * [spin.twin] crystal twin parameters
    % * [spin.matrix] 3x3 matrices for using them in the Hailtonian
    % * [spin.single_ion] single ion terms of the Hamiltonian
    % * [spin.coupling] list of bonds
    % * [spin.mag_str] magnetic structure
    % * [spin.unit] physical units for the Hamiltonian
    % * [spin.cache] temporary values
    %
    % ### Methods
    %
    % Methods are the different commands that require a `spin` object as a
    ...
    % ### See also
    %
    % [spin.copy], [spin.struct], [Comparing handle and value classes](https://www.mathworks.com/help/matlab/matlab_oop/comparing-handle-and-value-classes.html)
    %
    ...
    properties (SetObservable)
        ...
        %   [spin.unit] (default value is \\ang).
        %
        % `angle`
        ...
        % bond, the unit of energy is stored in [spin.unit] (default value 
        % is meV).
        %
        % `color`
        ...
        %   assignes one of the matrices from the [spin.matrix] property
        %   to a magnetic atom in the generated [spin.matom] list as a single
        ...
        %   assignes one of the matrices from the [spin.matrix] property
        %   to a magnetic atom in the spin.matom list as a
        ...
        %   unit is defined in [spin.unit] (default unit is Tesla).
        %
        % `T`
        % : Temperature, scalar, unit is defined in [spin.unit] (default
        %   unit is Kelvin).
        %
        single_ion
        ...
        %   magnetic atoms in the list generated by `spin.matom`, stored in a
    ...
    methods (Hidden = true)
       varargout = plot_base(obj,varargin);
    end
        
        
    methods
        function obj = spin(varargin)
            % spin constructor
            %
            % ### Syntax
            %
            ...
            % ### Description
            %
            % `obj = spin` creates an empty spin object with default
            % values.
            %
            % `obj = spin(struct)` creates a spin object from a structure
            % which has fields that are compatible with the spin property
            % structure.
            %
            % `obj = spin(hFigure)` clones spin object from an swplot
            % figure or spectral plot figure.
            %
            % `obj = spin(fName)` imports the file referenced by `fName`.
            % spin is able to import .cif/.fts files for crystal or
            % magnetic structure from a local file or a web address.
            %
            % `obj = spin(obj)` checks the input spin object for
            % consistency.
            %
            ...
            if ishandle(firstArg)
                % get spin object from graphics handle
        ...
        function clearcache(obj, chgField)
            ...
            % It is not necessary to clear the cache at any point as spin
            ...
            % listening to changes of the spin object to clear cache is
            % necessary
            if nargin<2
