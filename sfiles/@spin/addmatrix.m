...
% [spin.matrix] field of `obj`. The added matrices can be later assigned
...
%   following shapes:
%   * $[3\times 3]$ the given values will be stored in [spin.matrix] as
%     they are given.
...
% ### Output Arguments
% 
% The `obj` output will contain the additional matrix in the [spin.matrix]
% field.
% 
% ### See Also
% 
...
newMat = s_readparam(inpForm, varargin{:});

if ~isnumeric(newMat.value) && ~isa(newMat.value,'sym')
    warning('spin:addmatrix:WrongInput','Matrix value has to be numeric or symbolic variable!')
    return
end
...
if isempty(newMat(1).mat)
    ...
    nJ = nLabel;
    if nJ == 0
        error('spin:addmatrix:WrongInput','Define some matrix property!');
    else
        newMat.mat = repmat(eye(3),[1 1 nJ]);
        if ~obj.symbolic
            warning('spin:addmatrix:NoValue','No valid value was given for the new matrix, default value used!');
        end
    end
end
...
for ii = 1:size(newMat.mat,3)
    ...
    %spin.validate(newObj,'matrix');
    
    cIdx = find(strcmp(obj.matrix.label,newJItem.label));
    
    if numel(cIdx)>1
        error('spin:addmatrix:LabelError','There are multiple matrices with the same label!');
