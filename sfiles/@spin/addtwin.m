...
% the crystallographic twins are knows, Spin simulations can be directly
...
% ### Output Arguments
% 
% The function adds extra entries to the [spin.twin] property.
% 
% ### See Also
% 
% [spin] \| [spin.twinq]
...
if any(param.axis)
    ...
    for ii = 1:nTwin
        [~, param.rotC(:,:,ii)] = s_rot(param.axis,param.phi(ii));
    end
else
...
if (size(param.rotC,1)~=3) || (size(param.rotC,2)~=3)
    error('spin:addtwin:WrongInput','rotC matrix dimensions have to be [3 3 nTwin]!');
end
