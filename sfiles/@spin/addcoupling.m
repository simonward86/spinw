...
%   compared to [spin.unit_cell] where all symmetry inequivalent atoms are
...
% ### Output Arguments
% 
% The function adds extra entries to the [spin.coupling] property of
% `obj`. Specifically it will modify `obj.coupling.mat_idx`,
% `obj.coupling.type` and `obj.coupling.sym` matrices.
% 
% ### See Also
% 
% [spin] \| [spin.gencoupling] \| [spin.addmatrix]


if isempty(obj.matom.r)
    error('spin:addcoupling:NoMagAtom','There is no magnetic atom in the unit cell with S>0!');
end
...
if iscell(param.type)
    ...
    if any(isnan(param.type))
        error(['spin:addcoupling:WrongInput','Wrong coupling type, '...
...
if any(size(param.mat)~=size(param.type))
    error('spin:addcoupling:WrongInput',['A coupling type has to be '...
...
if ~isnumeric(param.atom)
    ...
    if numel(param.atom)>2
        error('spin:addcoupling:WrongInput','Only two different atom label can be given at once!');
    end
...
if ~isempty(param.atom) || ~isempty(param.subIdx)
    if numel(param.bond) > 1
        warning('spin:addcoupling:CouplingSize',['bond parameter has to be '...
            'scalar, only the first bond is selected!']);
        param.bond = param.bond(1);
    end
    %     if obj.sym
    %         error('spin:addcoupling:SymmetryProblem',['atom and subIdx options are not allowed '...
...
if ~any(idx)
    error('spin:addcoupling:CouplingError',['Coupling with idx=%d does '...
...
if obj.symmetry
    if obj.coupling.idx(max(idx)) > obj.coupling.nsym
        warning('spin:addcoupling:SymmetryLimit','The assigned bonds are not generated using space group symmetry!');
    end
end
...
if isempty(idx)
    warning('spin:addcoupling:NoBond','No matrix assigned, since no bond fulfilled the given conditions!')
    return
end
...
if any(ismember(Jmod(:),param.mat))
    warning('spin:addcoupling:CouplingIdxWarning',['Same matrix already '...
        'assigned on some coupling, duplicate assigments are removed!']);
end

if isempty(param.mat)
    warning('spin:addcoupling:WrongMatrixLabel','The selected matrix label does not exists!')
    return
end

if any(Jmod(3,:))
    error('spin:addcoupling:TooManyCoupling',['The maximum '...
...
if obj.symmetry && nosympar && any(~param.sym)
    warning('spin:addcoupling:SymetryLowered','By subselecting equivalent bonds, the symmetry of the corresponding bond(s) are reduced to P1!');
end
