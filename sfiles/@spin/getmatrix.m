...
% 
% `bond`
% : Index of the bond in `spin.coupling.idx`, e.g. 1 for first neighbor
%   bonds.
% 
% `aniso`
...
if ~obj.symmetry
    ...
    if isempty(obj.lattice.sym)
        warning('spin:getmatrix:NoSymmetry','The Spin object does not contain symmetry!')
    else
        warning('spin:getmatrix:NoSymmetry','Use spin.gencoupling first to generate bonds!')
    end
    return
end
...
if ischar(param.aniso)
    ...
    elseif numel(anisoIdx) == 1
        ...
        error('spin:getmatrix:WrongInput','The given atom label is not unique!')
    end
else
...
if ischar(param.mat) 
    ...
    if isempty(matIdx)
        error('spin:getmatrix:WrongInput','Matrix label cannot be found (case sensitive)!');
    elseif numel(matIdx) > 1
        error('spin:getmatrix:WrongInput','Multiple identical matrix labels exist!');
    end
else
    matIdx = param.mat;
end
...
if matIdx ~= 0
    ...
    if isempty(Cpidx) && isempty(Anidx) && isempty(Gidx)
        error('spin:getmatrix:WrongInput','Matrix is not assigned to any coupling/anisotropy/g-tensor!');
    elseif sumNum > 1
        error('spin:getmatrix:WrongInput','Matrix is assigned to multiple coupling/anisotropy/g-tensor!');
    else
        if ~isempty(Cpidx)
            bondIdx = Cpidx;
...
if bondIdx ~= 0
    ...
    if isempty(iSel)
        error('spin:getmatrix:WrongInput','The given bond index does not existst, check your input!');
    end
    
    
    if param.subIdx>numel(iSel)
        error('spin:getmatrix:WrongInput','The given subIdx reaches the number of bonds within the given symmetry (%d)!',numel(iSel));
    end
...
elseif anisoIdx ~= 0
    ...
    if isempty(matIdx0)
        error('spin:getmatrix:WrongInput','The given obj.matom.idx does not exists, check your input!');
    end
    if matIdx0(1) > 0
        matIdx = matIdx0(1);
    end
...
%[aMat, aSym] = s_basismat(pOp,dr(:,1));
[aMat, aSym] = s_basismat(pOp,A*dr(:,1));
...
if iscell(param.pref)
    ...
    elseif numel(param.pref{1}) == 3
        ...
        if sum(aSym) == 0
            error('spin:getmatrix:NoAsym','No asymmmetric matrix is allowed by symmetry!');
        elseif sum(aSym) > 3
            error('spin:getmatrix:SymError','Error of point group symmetry!');
        elseif sum(aSym) < 3
            warning('spin:getmatrix:Asym','Less than 3 assymetric matrices are allowed by symmetry!');
        end
        pref(aSym) = param.pref{1}(1:sum(aSym));
    else
        error('spin:getmatrix:WrongInput','Wrong value of pref, see help!');
    end
    param.pref = pref;
end
...
if numel(param.pref) > 1
    if numel(param.pref)~=size(aMat,3)
        error('spin:getmatrix:WrongInput','Wrong number of elements in pref option!');
    end
