...
% that can contain a name of an atom or its atom index (see [spin.atom]).
...
% ### See Also
% 
% [spin], [spin.single_ion], [spin.addcoupling], [spin.addg] and [spin.addmatrix]
%
...
if nMagAtom==0
    error('spin:addaniso:NoMagAtom','There is no magnetic atom in the unit cell with S>0!');
end
...
if matrixIdx == -1
    error('spin:addaniso:WrongCouplingTypeIdx','Input matrix does not exists!');
end

if nargin > 3
    atomIdx = varargin{2};
    if obj.lattice.sym > 1
        error('spin:addaniso:SymmetryProblem','atomIdx is not allowed when crystal symmetry is higher than P1!');
    end
...
if nargin > 2
    ...
    if iscell(atomTypeIdx)
        ...
        for ii = 1:numel(atomTypeIdx)
            ...
            if numel(sAIdx) == 0
                error('spin:addaniso:WrongString','Atom label doesn''t exist!')
            %elseif numel(sAIdx) > 1
            %    error('spin:addaniso:WrongString','Multiple atoms have the same label!')
            end
