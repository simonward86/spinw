...
%   the magnetic atoms in [spin.unit_cell], this will assign the given
...
% ### See Also
% 
% [spin] \| [spin.addcoupling] \| [spin.addaniso] \| [spin.addmatrix]
%
...
%
% See also spin, spin.addcoupling, spin.addaniso, spin.addmatrix.
%
...
if nMagAtom==0
    error('spin:addg:NoMagAtom','There is no magnetic atom in the unit cell with S>0!');
end
...
if matrixIdx == -1
    error('spin:addg:WrongCouplingTypeIdx','Input matrix does not exists!');
end

if nargin > 3
    atomIdx = varargin{2};
    if obj.lattice.sym > 1
        error('spin:addg:SymmetryProblem','atomIdx is not allowed when crystal symmetry is not P1!');
    end
