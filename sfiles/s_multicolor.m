function cMat = s_multicolor(vMat, cMap, cLim, nCol, cflip)
% overlays monochrome maps into a single RGB map
% 
% ### Syntax
% 
% `cmat = s_multicolor(vmat, cmap, clim, {ncol}, {flipud})`
% 
% ### Description
% 
% `cmat = s_multicolor(vmat, cmap, clim, {ncol}, {flipud})` takes 2D
...
% `rgbMap` and the Matlab built-in function `image`. We use `s_multicolor`
...
% >>C = s_multicolor(cat(3,A,B),[red green],[-1 1]);
% >>
% >>figure
% >>
% >>subplot(1,3,1)
